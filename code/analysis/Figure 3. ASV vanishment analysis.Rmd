---
title: An R Markdown document converted from "./Figure 3. ASV vanishment analysis.ipynb"
...
# calculate p-values
odds$pvalue <- sapply(odds$odds, function(x){1-sum(x>odds$odds)/length(odds$odds)})
...
p <- ggplot(odds, aes(x=odds)) + 
   ...
   annotate("text", x=4.5, y=900, 
            label= paste0("Quantile 95%:\nOdds = ", round(threshold, 2)), size=7, hjust = 0) + 
   xlim(0, 8.5) +
...
k = 0.3
ggsave(file=file.path('../../results/figures', 'supplementary_figure1', 'histogram.svg'), plot=p, 
...
```{r}
odds_table <- odds
odds_table$counts_absence <- as.vector(unlist(lapply(odds_table$values, function(x) strsplit(x, split=' ')[[1]][1])))
odds_table$counts_presence <- as.vector(unlist(lapply(odds_table$values, function(x) strsplit(x, split=' ')[[1]][2])))
odds_table <- odds_table[, c('genera', 'genera_name', 'counts_absence', 'counts_presence', 'odds', 'pvalue')]
colnames(odds_table) <- c('ASV', 'taxonomy', 'counts_absence', 'counts_presence', 'odds', 'pvalue')
odds_table <- list(Odds = odds_table)
write_xlsx(odds_table, file.path('../../results/tables', 
...
```{r}
sum(df_bar_trimmed_STHNegative$Counts)
sum(df_bar_trimmed_STHPositive$Counts)
```

```{r}
df_bar_ <- df_bar_trimmed_STHNegative
row.names(df_bar_) <- df_bar_trimmed_STHNegative$Family
df_bar_[df_bar_trimmed_STHPositive$Family, 'Counts'] <- 0
df_bar_[df_bar_trimmed_STHNegative$Family,  'Counts'] <- df_bar_trimmed_STHNegative$Counts
df_bar_[df_bar_trimmed_STHPositive$Family,  'Counts'] <- df_bar_[df_bar_trimmed_STHPositive$Family, 'Counts'] + df_bar_trimmed_STHPositive$Counts
df_bar_$fraction <- df_bar_$Counts/sum(df_bar_$Counts)
df_bar_[order(-df_bar_$fraction), c('Counts', 'fraction')]
```
...
add_counts <- dplyr::filter(df_bar_trimmed, (df_bar_trimmed$Phylum == "Bacillota")|(df_bar_trimmed$Phylum == "Bacteroidota"))
# calculate the total counts of Firmicutes Phylum
add_counts <- aggregate(Counts ~ group, add_counts, sum)
# ensure an order by group
add_counts <- add_counts[order(add_counts$group), ]
# add counts to the first Families of the Bacteroides family 
df_bar_trimmed[c('Gastranaerophilaceae;STHNegative'), 'Counts'] <- df_bar_trimmed[c('Gastranaerophilaceae;STHNegative'), 'Counts']+add_counts$Counts[1]

# add Firmicutes Phylum counts to the first Families of the Bacteroides family 
# in STHNegative and STHPositive groups
# select Firmicutes Phylum counts
add_counts <- dplyr::filter(df_bar_trimmed, df_bar_trimmed$Phylum == "Bacillota")
# calculate the total counts of Firmicutes Phylum
add_counts <- aggregate(Counts ~ group, add_counts, sum)
# ensure an order by group
add_counts <- add_counts[order(add_counts$group), ]
# add counts to the first Families of the Bacteroides family 
df_bar_trimmed[c('Bacteroidaceae;STHNegative', 'Bacteroidaceae;STHPositive'), 'Counts'] <- df_bar_trimmed[c('Bacteroidaceae;STHNegative', 'Bacteroidaceae;STHPositive'), 'Counts']+add_counts$Counts
```
...
p <- ggplot(data=df_bar_trimmed) +
     ...
     scale_fill_manual(values=c('#e5e5e5', '#e5e5e5'))+
      new_scale_fill() +
      geom_bar(data = ~ dplyr::filter(., Phylum == "Cyanobacteriota") %>% dplyr::rename(Cyanobacteriota = Family), 
              mapping = aes(x=group, y=Counts, fill=Cyanobacteriota), 
              stat="identity") +
      scale_fill_manual(values=c('#329b96'))+
      new_scale_fill() +
      geom_bar(data = ~ dplyr::filter(., Phylum == "Bacteroidota") %>% dplyr::rename(Bacteroidota = Family),  
              mapping = aes(x=group, y=Counts, fill=Bacteroidota), 
              stat="identity") +
      scale_fill_manual(values=rev(c('#dbb99d', '#d19564', '#b47e54', '#773501')))+
      new_scale_fill() +
      geom_bar(data = ~ dplyr::filter(., Phylum == "Bacillota") %>% dplyr::rename(Bacillota = Family),  
              mapping = aes(x=group, y=Counts, fill=Bacillota), 
              stat="identity") +
      scale_fill_manual(values=rev(c('#d8bddc',  '#ac96d2', '#9071aa', '#6d4b88', '#5c396f', '#3e2248', '#9467bd')))+
      new_scale_fill() +
