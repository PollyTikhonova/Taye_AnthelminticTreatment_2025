---
title: An R Markdown document converted from "./Supplementary Figure 4. ALDEx2 analysis.
  Wilcoxon with covariates.ipynb"
...
```{r}
metadata$sample_eggcountAL_bin <- metadata$sample_eggcountAL != 0
metadata$sample_eggcountTT_bin <- metadata$sample_eggcountTT != 0
metadata$sample_eggcountHW_bin <- metadata$sample_eggcountHW != 0
```

```{r}
selected_sampels <- row.names(metadata)[rowSums(is.na(metadata[, c('sample_eggcountAL_bin',
                                                    'sample_eggcountTT_bin',
                                                    'sample_eggcountHW_bin'
                                                                  )])) == 0]
metadata <- metadata[selected_sampels, ]
```
...
selected_samples <- intersect(row.names(group_baseline), row.names(group_followup))
...
group_baseline <- group_baseline[selected_samples, ]
group_followup <- group_followup[selected_samples, ]
...
ps_clr <- prune_samples(row.names(metadata_aldex2), ps_clr)

otu_data_clr <- data.frame(otu_table(ps_clr))
otu_data_clr <- otu_data_clr[row.names(metadata_aldex2), ]
```

```{r}
# residuals for microbial genera
otu_data_residuals <- lapply(colnames(otu_data_aldex2), function(genera){
    glm_res <- glm(otu_data_clr[[genera]] ~ sample_eggcountAL_bin+sample_eggcountTT_bin+sample_eggcountHW_bin, data = metadata_aldex2)
    res <- data.frame(glm_res$residuals)
    colnames(res) <- c(genera)
    return(res)
})
otu_data_residuals_raw <- do.call('cbind', otu_data_residuals)
otu_data_residuals <- (otu_data_residuals_raw - min(otu_data_residuals_raw))*100
otu_data_residuals <- as.data.frame(lapply(otu_data_residuals, as.integer), 
                                    row.names=row.names(metadata_aldex2))
otu_data_residuals_raw <- as.data.frame(otu_data_residuals_raw, 
                                    row.names=row.names(metadata_aldex2))
```
...
asv.clr <- aldex.clr(t(as.matrix(otu_data_residuals)), metadata_aldex2[['collection_timepoint']])
```
...
```{r}
# log2 transform
otu_data_log <- otu_data_residuals
otu_data_log <- otu_data_log/rowSums(otu_data_log)
otu_data_log <- log2(otu_data_log)
# compute folds
...
log2fold[abs(log2fold) == Inf] <- NA
log2fold_mean = colMeans(log2fold, na.rm = TRUE)
```
...
#     legendPosition = 'right',
    ...
    title = bquote("Samples: all subjects"),
    subtitle = "ALDEx2 Wilcoxon test with covariates:\nBaseline vs 1-year follow-up.",
    titleLabSize = 24,
    subtitleLabSize = 22,
    caption = "") + #"#b8aa10" 
...
# save the plot
k = 0.3
ggsave(file=file.path('../../results/figures', 'supplementary_figure4', 'ALDEx2.Wilcoxon_test_covariates.volcano_plot.svg'), plot=p, 
...
otu_data_boxplot <- otu_data_residuals_raw[, aldex2_res[aldex2_res$padj <= 0.05, ]$lab]
...
# add titles
      labs(x = "", y = "Residuals", title = "Samples: all subjects",
           subtitle = "ALDEx2 paired Wilcoxon test with covariates:\nBaseline vs 1-year follow-up.") +
...
k = 0.3
ggsave(file=file.path('../../results/figures', 'supplementary_figure4', 
                      'ALDEx2.Wilcoxon_test_covariates.boxplots.all.svg'), plot=p, 
       width=width*k, height=height*k, scaling = k)
print(p)
```
...
otu_data_residuals <- data.frame(otu_table(ps_clr))
otu_data_residuals <- otu_data_residuals[row.names(metadata_aldex2), ]
```

```{r}
# residuals for microbial genera
otu_data_residuals <- lapply(colnames(otu_data_aldex2), function(genera){
    glm_res <- glm(otu_data_residuals[[genera]] ~ sample_eggcountAL_bin+sample_eggcountTT_bin+sample_eggcountHW_bin, data = metadata_aldex2)
    res <- data.frame(glm_res$residuals)
    colnames(res) <- c(genera)
    return(res)
})
otu_data_residuals <- do.call('cbind', otu_data_residuals)
otu_data_residuals <- (otu_data_residuals - min(otu_data_residuals))*100
otu_data_residuals <- as.data.frame(lapply(otu_data_residuals, as.integer))
```
...
k = 0.3
ggsave(file=file.path('../../results/figures', 'supplementary_figure4',
                      'ALDEx2.Wilcoxon_test_covariates.noSTH.volcano_plot.svg'), plot=p, 
       width=width*k, height=height*k, scaling = k)
```
...
k = 0.3
ggsave(file=file.path('../../results/figures', 'supplementary_figure4',
                      'ALDEx2.Wilcoxon_test_covariates.noSTH.boxplots.all.svg'), plot=p, 
       width=width*k, height=height*k, scaling = k)
print(p)
```
...
k = 0.3
ggsave(file=file.path('../../results/figures', 'supplementary_figure4', 
                      'ALDEx2.Wilcoxon_test_covariates.STH.volcano_plot.svg'), plot=p, 
       width=width*k, height=height*k, scaling = k)
```
...
k = 0.3
ggsave(file=file.path('../../results/figures', 'supplementary_figure4', 
                      'ALDEx2.Wilcoxon_test_covariates.STH.boxplots.all.svg'), plot=p, 
       width=width*k, height=height*k, scaling = k)
print(p)
```
...
write_xlsx(p_values_summary_table, file.path('../../results/tables', 
                                             'Table S4.ALDEx2.Wilcoxon_test_with_covariates.xlsx'))
```
