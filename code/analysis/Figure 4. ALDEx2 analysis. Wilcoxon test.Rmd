---
title: An R Markdown document converted from "./Figure 4. ALDEx2 analysis. Wilcoxon
  test.ipynb"
...
otu_data_boxplot$variable <- factor(otu_data_boxplot$variable, 
                                    levels=c('Senegalimassilia', 'Blautia', 'Marvinbryantia'))
```
...
# change default colors
      ...
      scale_colour_manual(values = c('#878787', '#b9a04f')) +
# add titles
      labs(x = "", y = "Relative abundance", title = bquote("Samples: all subjects"),
           subtitle = "ALDEx2 paired Wilcoxon test:\nBaseline vs 1-year follow-up.") +
# ensure the maximum y axis limit is high enough to fit all the neccesary info 
      geom_blank(data = otu_data_max, aes(x = grouping, y=value, group = host_subject_id)) +
# expand y limits to fit the p-value statistics into a plot
...
# change default colors
      ...
      scale_colour_manual(values = c('#878787', '#b9a04f')) +
# add titles
      labs(x = "", y = "Relative abundance", title = bquote("Samples: -STH at baseline"),
           subtitle = "ALDEx2 paired Wilcoxon test:\nBaseline vs 1-year follow-up.") +
# expand y limits to fit the p-value statistics into a plot
      scale_y_continuous(expand = expansion(mult = 0.03)) +
# ensure the maximum y axis limit is high enough to fit all the neccesary info 
...
otu_data_boxplot <- melt(otu_data_boxplot)
otu_data_boxplot$variable <- factor(otu_data_boxplot$variable, 
                                   levels=c('Senegalimassilia', 'Collinsella', 'Intestinimonas'))
```
...
write_xlsx(p_values_summary_table, file.path('../../results/tables', 'Table S3.ALDEx2.Wilcoxon_test.xlsx'))
```
